...
function tryboxEdit() {
    if (trybox == 1) {
        let strongBox = document.createElement("strong");
        strongBox.innerHTML = "...";
        strongBox.setAttribute('id', 'str_id');
        document.querySelector('#number-coin-luckydraw').appendChild(strongBox);
        document.querySelector('#img_pop').setAttribute('src', 'img/daily_lucky.png')
        document.querySelector('#reward_box_text').innerHTML = lang['rewardbox_text'];
        document.querySelector('#des_box').innerHTML = lang['rewardbox_content_complete'];
        document.querySelector('#btn_failer').innerHTML = lang['rewardbox_btn_success'];
        document.querySelector('#aff_link').setAttribute('href', `${lang['rewardbox_btn_link']}`);
        // setTimeout(() => {
        //     papers_winn();
        // }, 11500)
    } else if (trybox > 1) {
        location.replace(lang['rewardbox_btn_link']);
    } else {
        document.querySelector('#reward_box_text').innerHTML = lang['rewardbox_false'];
        document.querySelector('#des_box').innerHTML = lang['rewardbox_content_failed'];
        document.querySelector('#btn_failer').innerHTML = lang['rewardbox_btn_failed'];
        document.querySelector('#img_pop').setAttribute('src', 'img/gift_box.png');
        if (document.querySelector('#str_id') !== null) {
            str_id.remove()
        }
    }
    trybox++;
...
function spin(c, e) {
    if (!localStorage.getItem("remainTime")) {
        ...
        outerSpin.radius("12%");
        innerSpin.radius("8%");
        spinImage.finish(tryboxEdit()).size(140).move(_centerX / 1.23, _centerY / 1.22);
        ...
        _globalVars.elms.rotateGroup.animate(5E3, "circInOut").rotate(3600 - b).afterAll(function() {
            setTimeout(function() {
                _globalVars.elms.pizza.children()[f - 1].animate(100).fill("#FFF").loop(15, !0).afterAll(function() {
...
var cachedKey = "version" + window.location.hostname;
...
function drawGraphic() {
    ...
    for (h = 0; h < _globalVars.config.totalSlices; h++) {
        ...
        12 === parseInt(_globalVars.config.totalSlices) && (b.opacity(0), renderImageReward(c,
            h, e.x1, e.y1));
